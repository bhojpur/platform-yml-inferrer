...
export class ConfigInferrer {
    ...
    async getConfig(ctx: Context): Promise<ApplicationConfig> {
    ...
    protected addCommand(config: ApplicationConfig, command: string, phase: 'before' | 'init' | 'command', unless?: string): void {
        if (!config.tasks) {
            config.tasks = [];
        }
        if (!config.tasks[0]) {
